...
void X86PassConfig::addPreRegAlloc() {
  ...
  addPass(createX86WinAllocaExpander());
	...
	addPass(createX86ShadowStackMPX());
#endif
}
...
void X86PassConfig::addPreEmitPass() {
  ...
  if (getOptLevel() != CodeGenOpt::None) {
	...
	addPass(createX86FunctionCountPass());
#endif
#ifdef SHADOW_STACK_REG
    addPass(createX86ShadowStackReg());
#endif

#ifdef SHADOW_STACK_MEM_SCHEME
	addPass(createX86ShadowStackMem());
#endif

#ifdef SHADOW_STACK_SEG
	addPass(createX86ShadowStackSeg());
#endif

#ifdef SHADOW_STACK_CON
	addPass(createX86ShadowStackCon());
#endif
}
